...
function get_genre ($content) {
  var text = $content('#titleStoryLine > div').filter(function () {
    return $content(this).text().trim().startsWith('Genres:')
    }).text().trim()
    if (text) {
    text = text.split(':', 2)[1].split(',').map((e) => {
      if (e.includes('»'))
...
module.exports = {
  ...
  search_imbd_movie_by_imdb_id: search_imbd_movie_by_imdb_id,
